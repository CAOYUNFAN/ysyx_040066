...
static inline void add_aw(){
  ...
  if(write_table.addr>=0x80000000u){
...
static inline void deal_w(){
  ...
  write_table.left--;
  if(!write_table.left) Assert(mycpu->io_master_wlast,"Write expect to be ended! %x",write_table.addr);
  else Assert(!mycpu->io_master_wlast,"Write expect not to be ended! %x",write_table.addr);
